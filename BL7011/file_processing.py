...
def read_asc(path: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Reads data from an asc file generated by the ALV-7004 correlator used
    for PCS characterization at the ALS. Returns the data as three pandas
    ...
    Returns a tuple containing...
        data_correlation: pd.DataFrame | None
            Correlation data, stored with a column for tau [ms] and correlation
        data_count_rate: pd.DataFrame | None
            Count rate data, stored with a column for count time [s] and
            count rate [kHz]
        data_instrument: pd.DataFrame | None
    ...
    with open(path, mode='r', encoding='latin-1') as data_file:
    ...
    return data_correlation, data_count_rate, data_instrument